...
class GcHelpers extends \System
{
    ...
    public static function createNewImage($intAlbumId, $strFilepath)
    {
        ...
        $strAlbumAlias = $objAlbumModel->alias;
        ...
        if (!$objAlbumModel->preserve_filename && $blnExternalFile === false)
        {
    ...
    public static function getAlbumInformationArray($intAlbumId, $objContentElement)
    {
        ...
        if (TL_MODE == 'FE')
        {
            ...
            $href = ampersand($objPageModel->getFrontendUrl((\Config::get('useAutoItem') ? '/%s' : '/items/%s')));
            // Add albumAlias
    ...
    public static function getPictureInformationArray($intPictureId = null, $objContentElement)
    {
        ...
        $arrPicture = array(
            ...
            'href' => $href,
            // single image url
            'single_image_url' => ampersand($objPageModel->getFrontendUrl((\Config::get('useAutoItem') ? '/' : '/items/') . \Input::get('items') . '/img/' . $arrFile["filename"])),
